  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6 flex flex-col h-full transition-transform transform hover:-translate-y-1 hover:shadow-2xl">
      <div className="mb-3">
        ...
        {schedule.mode === 'bulk' ? (
          <p className="text-black text-sm flex items-center mt-2">
            <ClockCircle className="mr-2 w-4 h-4" /> {formatTime(schedule.startTime)} -{' '}
          ...
          <div className="mt-auto flex items-center justify-between">
            <div className="flex items-center space-x-2">
              <Switch
                ...
                onCheckedChange={() => onToggleActive && onToggleActive(schedule)}
            ...
            <div className="flex items-center space-x-2">
              <Button variant="ghost" onClick={() => onEdit && onEdit(schedule)}>
                {t('Card.Modify')}
              </Button>
              <Button variant="destructive" onClick={() => onDelete && onDelete(schedule)}>
                {t('Card.Delete')}
              </Button>
            </div>
          </div>
