  ...
  useEffect(() => {
    const fetchVisits = async () => {
      try {
        ...
        console.error('Errore durante il caricamento dei tipi di visita:', error)
      }
    }
    fetchVisits()
  }, [onVisitsChange])
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-6 justify-items-center max-w-screen-xl mx-auto">
      {visits.map((visit, index) => (
        <div
          ...
          className={`cursor-move ${dragOverItemIndex === index ? 'border-2 border-blue-500' : ''}`}
          style={{
        ...
        >
          <CardItem visit={visit} showButton={showButton} setIsModalOpen={setIsModalOpen} />
        </div>
      ))}
    </div>
  )
